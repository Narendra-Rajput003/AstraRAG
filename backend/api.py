...
from backend.auth import authenticate_user, generate_tokens, store_refresh_token, initialize_auth_db, require_auth, require_role, create_invite, register_user, setup_mfa, verify_mfa, enable_mfa
from backend.audit import log_audit_event
from backend.rag_pipeline import load_and_chunk_docs, create_vector_store
from backend.search import search_service
from backend.analytics import analytics_service
from backend.collaboration import collaboration_service, handle_collaboration_event
...
async def create_user_invite(request: InviteRequest, user: dict = Depends(require_role(['superadmin'])), req: Request = None):
...
async def register_new_user(request: RegisterRequest, req: Request = None):
...
async def revoke_invite(invite_id: str, user: dict = Depends(require_role(['superadmin'])), req: Request = None):
    ...
    try:
        ...
        if PRESIDIO_AVAILABLE:
            try:
                ...
                anonymizer.anonymize(text=extracted_text, analyzer_results=results)
...
async def approve_document(request: ApproveDocumentRequest, user: dict = Depends(require_role(['superadmin'])), req: Request = None):
