...
try:
    ...
    DIRECTORY_LOADER_AVAILABLE = True
except ImportError:
    DirectoryLoader = None
    UnstructuredPDFLoader = None
    DIRECTORY_LOADER_AVAILABLE = False
...
from backend.core.security import mask_pii, add_security_metadata
...
async def load_and_chunk_docs(directory: str) -> List[Any]:
    ...
    if not DIRECTORY_LOADER_AVAILABLE or not TEXT_SPLITTER_AVAILABLE or not LANGCHAIN_AVAILABLE:
    ...
    if DirectoryLoader is None or UnstructuredPDFLoader is None:
        logger.warning("DirectoryLoader or UnstructuredPDFLoader not available")
        return []
    ...
    for chunk in chunks:
        ...
        if LANGCHAIN_AVAILABLE and hasattr(Document, '__init__') and Document is not Any:
            ...
            masked_chunk = {"page_content": masked_content, "metadata": chunk.metadata}
...
def setup_retriever(vector_store: Any, llm: Any, user_id: str) -> Any:
    ...
    if PromptTemplate is None:
        logger.warning("PromptTemplate not available")
        return None
    ...
    if CohereRerank is None:
        logger.warning("CohereRerank not available")
        return None
