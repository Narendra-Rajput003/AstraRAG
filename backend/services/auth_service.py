...
def register_user(email: str, password: str, invite_token: str, client_ip: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        """, (user[0] if user else None, invite_data["invite_id"]))
        ...
        return {
            "user_id": str(user[0]) if user else "",
...
async def register(request: RegisterRequest, req: Optional[Request] = None):
...
async def create_user_invite(request: InviteRequest, req: Optional[Request] = None):
...
async def revoke_invite(invite_id: str, req: Optional[Request] = None):
    """Revoke an invite."""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
...
async def setup_user_mfa(req: Optional[Request] = None):
...
async def verify_user_mfa(request: MFAVerifyRequest, req: Optional[Request] = None):
    """Verify MFA setup."""
    user_id = req.headers.get("X-User-ID") if req else None
    if not user_id:
        raise HTTPException(status_code=401, detail="User not authenticated")
...
async def authenticate_mfa(request: MFAVerifyRequest, req: Optional[Request] = None):
...
async def validate_token_endpoint(req: Optional[Request] = None):
